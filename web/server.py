...
def _ensure_worker():
    ...
    if not _worker_started:
        return  # don't auto-start on load — user must explicitly submit or resume
...
def submit_downloads(req: DownloadRequest):
    """Enqueue one or more TikTok URLs."""
    ...
    seen_in_req: set = set()
    with _queue_lock:
        # collect already-queued URLs to avoid batch dupes
...
async def downloads_stream():
    """SSE stream — sends queue state on connect, then live updates."""
...
@app.delete("/api/downloads/queue/{item_id}")
...
@app.post("/api/downloads/resume-queue")
def resume_full_queue():
    """Start the worker to process all waiting items — called after restart."""
    with _queue_lock:
        waiting = [i for i in _dl_queue if i["status"] == "waiting"]
    if not waiting:
        raise HTTPException(400, "No waiting items in queue")
    _start_dl_worker()
    return {"ok": True, "waiting": len(waiting)}


@app.post("/api/downloads/queue/{item_id}/resume")
def resume_queue_item(item_id: str):
    """Re-queue an interrupted item."""
    with _queue_lock:
        item = next((i for i in _dl_queue if i["id"] == item_id), None)
        if not item:
            raise HTTPException(404, "Queue item not found")
